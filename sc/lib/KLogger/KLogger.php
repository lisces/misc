...
class KLogger
{
   ...
   * @param string  $logName      File name to the logging file
   * @param integer $severity     One of the pre-defined severity constants
   * @return KLogger
   */
  public static function instance($logDirectory = false, $logName = null, $severity = false)
  {
    if ($severity === false) {
    ...
    if (in_array($logFile, self::$instances)) {
      return self::$instances[$logFile];
    }

    self::$instances[$logFile] = new self($logDirectory, $logName, $severity);
   ...
   * @return void
   */
  public function __construct($logDirectory, $logName, $severity)
  {
    ...
    $this->_logFilePath = empty($logName)
      ? $logDirectory . DIRECTORY_SEPARATOR . 'log_' . date('Y-m-d') . '.log'
      : $logDirectory . DIRECTORY_SEPARATOR . $logName . '_' . date('Y-m-d') . '.log';
    ...
    if (is_file($this->_logFilePath) && !is_writable($this->_logFilePath)) {
  ...
  {
    $time = date(self::$_dateFormat) . '(' . time() . ')';
